...
def create_cloudformation_stack(stack_name, template_body, parameters=None):
    """
    Creates an AWS CloudFormation stack.

    Args:
        stack_name (str): The name of the CloudFormation stack.
        template_body (str): The CloudFormation template content (JSON or YAML).
        parameters (list, optional): A list of dictionaries for stack parameters.
                                      Each dictionary should have 'ParameterKey' and 'ParameterValue'.
    Returns:
        dict: The response from the create_stack API call.
    """
...
if __name__ == "__main__":
    # Example CloudFormation template (in JSON format)
    # This template creates an S3 bucket
    sample_template = """
    {
      "AWSTemplateFormatVersion": "2010-09-09",
      "Description": "A simple S3 bucket CloudFormation template",
      "Resources": {
        "MyS3Bucket": {
          "Type": "AWS::S3::Bucket",
          "Properties": {
    ...
    stack_name = "MySampleS3Stack"
    
    # You can also load the template from a file
    # with open('my_template.json', 'r') as f:
    #     sample_template = f.read()

    # Example parameters (if your template has parameters)
    # stack_parameters = [
    #     {
    #         'ParameterKey': 'BucketNameParameter',
    #         'ParameterValue': 'my-custom-bucket-name'
    #     }
    # ]

    # Create the CloudFormation stack
    create_cloudformation_stack(stack_name, sample_template) #, stack_parameters)